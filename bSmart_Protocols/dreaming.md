...
- Dreaming must also stay separate from **runtime onboarding and deployment changes**. It may detect and report a missing Hermes `/project` integration, or create a pending setup item for the next interactive session, but it must not copy plugins, edit Hermes configuration, restart Hermes, deploy, or modify host/runtime state unattended.
